...
class TestPlanConformsToGitopsSchema:
    ...
    @pytest.mark.anyio
    async def test_low_risk_plan_valid(self, gitops_execution_plan_schema: dict[str, Any]) -> None:
        orchestrator = Orchestrator(
            settings=_settings(), github_pr=None, event_store=InMemoryEventStore()
        )
...
class TestSchemaDrift:
    ...
    def test_action_types_match_revenue_agent(
        ...
        action_type_enum = action_def.get("properties", {}).get("action_type", {}).get("enum", [])
    ...
    def test_channel_enum_match(self, gitops_execution_plan_schema: dict[str, Any]) -> None:
        """Channel enum in gitops must include all channels revenue_agent uses."""
        action_def = gitops_execution_plan_schema.get("$defs", {}).get("action", {})
        channel_enum = action_def.get("properties", {}).get("channel", {}).get("enum", [])
